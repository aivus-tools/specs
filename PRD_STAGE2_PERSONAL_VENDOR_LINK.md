...
> Статус: фича реализована. Фактическое состояние as-built описано в `VENDOR_LINKS.md` (корень репозитория). Этот PRD сохраняется как исторический планировочный контракт и как product-contract baseline для `Specs/scripts/stage2_review.js`, поэтому не удаляется. Где первоначальный замысел разошёлся с реализацией, по тексту даны пометки «as-built (см. VENDOR_LINKS.md)».
...
> as-built (см. VENDOR_LINKS.md): промежуточного старт-экрана-заглушки нет. `/brief/<slug>` сразу рендерит `BrandedBriefWorkspace` с AI-диалогом, брендинг вендора (логотип и название) уходит в шапку. И `/brief/[slug]`, и `/brief/[slug]/[briefId]` используют общий `BrandedBriefWorkspace`; документ-панель появляется только когда бриф готов.
...
> as-built (см. VENDOR_LINKS.md): в Settings (`VendorSettingsSection`) живут только slug, lead-notification-email и брендинг. Embed и Webhook сведены в один попап «Embed & Webhook» (две вкладки) на дашборде через `PersonalLinkPanel`/`EmbedWebhookModal`, а не в Settings. Вкладка Webhook показывает ключ, endpoint и готовые примеры cURL / JavaScript / Python.
...
> as-built (см. VENDOR_LINKS.md): кнопка на панели открывает единый попап «Embed & Webhook» (две вкладки), а не отдельный Embed-modal. URL ссылки и iframe-сниппет строятся от текущего домена через `usePublicAppOrigin` (`window.location.origin`), а не от захардкоженного `go.aivus.co`: локально это `localhost:3000`, на проде - реальный домен, на кастомном - его.
...
> as-built (см. VENDOR_LINKS.md): draft и `Project(vendor, brief, status=DRAFT)` создаются при отправке первого сообщения, не на заходе и не по кнопке старт-экрана. Возобновление прерванной сессии - через cookie `aivus_pending_brief` (TTL около часа), без связки localStorage + redirect.
...
> as-built (см. VENDOR_LINKS.md): ключ принимается в заголовке `X-Aivus-Webhook-Key` (предпочтительно) и, как fallback для платформ без кастомных заголовков, в JSON-теле (`key`/`webhookKey`). См. `_verify_vendor_webhook_key`.
...
> as-built (см. VENDOR_LINKS.md): отдельной карточки-заглушки «Start brief» нет. `/brief/<slug>` сразу открывает `BrandedBriefWorkspace` с AI-диалогом, брендинг (логотип + название) - в шапке. Документ-панель появляется только когда бриф готов.
...
> as-built (см. VENDOR_LINKS.md): Embed-сниппет и Webhook-ключ показываются не на дашборде отдельным Embed-modal и не в Settings, а в едином попапе «Embed & Webhook» (две вкладки: Embed - iframe-сниппет; Webhook - ключ показать/скопировать/перевыпустить, endpoint и примеры cURL / JavaScript / Python). В Settings остаются только slug, lead-notification-email и брендинг. URL и сниппет - origin-based (`usePublicAppOrigin`), без захардкоженного `go.aivus.co`.
...
> as-built (см. VENDOR_LINKS.md): `_verify_vendor_webhook_key(request, body)` принимает ключ из заголовка `X-Aivus-Webhook-Key` (предпочтительно) либо, как fallback, из JSON-тела (`key`/`webhookKey`) - для платформ автоматизации без кастомных заголовков.
