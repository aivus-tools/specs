...
Даём каждому вендору личную брендированную ссылку на форму брифа вида `go.aivus.co/brief/<имя>`. Вендор кладёт её на свой сайт, шлёт клиентам в переписке или встраивает прямо в страницу как виджет. Клиент открывает ссылку, видит логотип и название агентства (не наш бренд), проходит короткий диалог с AI-продюсером и одной кнопкой отправляет готовый бриф. Бриф сразу оказывается у вендора в кабинете как проект-лид. Клиенту не нужно заранее регистрироваться: после отправки ему на почту приходит письмо с двумя действиями — сразу скачать PDF или зарегистрироваться с этим email, чтобы бриф сохранился в кабинете со всеми функциями.
...
Что строим с нуля: slug-резолв и draft-by-slug, токенные final-doc эндпоинты + тонкий read-only preview, Send-флоу (async chain finalize→create vendor Project→emails), привязку Project к брифу в обход vendor-guard, vendor-read PDF, email-path без User-контекста, vendor-нотификацию, per-vendor webhook-ключ, CSP для `/brief/*`, доработку `_create_inbound_brief` чтобы реально писать атрибуцию.
...
- для АНОНИМА финальный документ показываем через новые токенные эндпоинты `public/.../final-documents` (read) и тонкий read-only `DocumentPreview` (у анонима своего вью документа сегодня нет — это добавление, не урезание);
...
6. AI доводит до `ready_to_finalize`. В white-label режиме промпт не толкает к регистрации (S2-21). Документ показываем read-only через токенный final-documents GET. Аноним меняет документ через чат с AI («подправь бюджет, сделай гибким») — для анонима прямого текстового редактирования на MVP нет (после регистрации он получает полный редактор, см. §4(г)).
...
3. Письмо клиенту даёт ДВА действия: (а) primary CTA «Зарегистрируйся с этим email — сохранишь бриф в кабинете со всеми функциями» и (б) прямую ссылку «Скачать PDF» (по токену, без регистрации — лид у вендора уже есть, держать PDF в заложниках незачем). Если email уже зарегистрирован → вместо CTA регистрации «Войди, чтобы открыть свой бриф», ссылка на PDF остаётся. Аккаунт заранее НЕ создаём (см. §12 п.15).
...
Это про АНОНИМНЫЙ white-label редактор. Двухпанельный на desktop (слева DocumentPreview, справа чат), на мобиле — табы (Brief / Chat). **DocumentPreview** — новый тонкий компонент: read-only рендер HTML из токенного final-documents GET (у анонима своего вью документа нет — добавляем). Аноним меняет документ через чат с AI.
...
**Публичные документы клиента (по токену, новый):**
- `GET /service/public/briefs/ai/<briefId>/final-documents?token=...` → HTML финальных документов (для read-only DocumentPreview анонима).
- `GET /service/public/briefs/ai/<briefId>/documents/<docId>/pdf?token=...` → отдаёт PDF без сессии (ссылка «Скачать PDF» из письма). PDF готов — письма шлёт chain после finalize. Токен — брифа; после claim токен очищается, ссылка на PDF перестаёт работать, но у клиента уже есть доступ через аккаунт. Реализация: либо отдельный долгоживущий download-токен, либо graceful 410 после claim.

**Webhook (новый):**
- `POST /service/public/briefs/ai/from-webhook`, заголовок `X-Aivus-Webhook-Key`. `_verify_vendor_webhook_key(request)` → vendor. `_create_inbound_brief(..., vendor, source="webhook")` — дорабатываем тело чтобы писать `source` на Brief и сразу создавать `Project(vendor, brief, status=RFP)`. Rate-limit `50/h` по vendor_id.
...
- Новые шаблоны: `brief_sent_client_{en,ru}.html` (ДВА действия: CTA регистрации с этим email + прямая ссылка «Скачать PDF» по токену; для existing email — «Войти» вместо регистрации), `vendor_lead_{en,ru}.html`.
...
| `views_brief_v3.py` | `by-slug` detail/draft; токенные public `final-documents` GET; `send` (public+client, async chain); `from-webhook`; `_verify_vendor_webhook_key`; доработка `_create_inbound_brief:1223`; ретрофит wix |
| `projects/tasks.py` | `create_vendor_project_task`, `send_emails_task` |
...
| `app/brief/[slug]/[briefId]/page.tsx` | редактор (AnonymousBriefEditor + DocumentPreview, whiteLabel, Send) |
| `app/brief/[slug]/success/page.tsx` | success |
| `modules/client/BriefEditor/DocumentPreview.tsx` | новый тонкий read-only preview |
...
- **Rate-limit**: `by-slug` 60/h по IP; draft/start — существующие; chat 5/m; Send 10/h по IP/брифу; webhook 50/h по vendor_id.
...
- DocumentPreview + whiteLabel: PDF/Share/PreVendors/vendor_email скрыты, Send показан и disabled до finalized.
...
10. АНОНИМНЫЙ превью документа (до регистрации) — read-only, правки через чат с AI. После логина — полный существующий редактор без изменений, клиент правит как угодно (это уже работает); white-label лишь прячет Download/Share/PreVendors на брендированной странице.
